...
- [x] **Task 14: Sub-exclusion flip-rate measurement (F1).** — DONE 2026-06-01. Ran against the prod DB (real `calculateHarveyPoints` in-container): 6 finalized official rounds, 3 with subs, **0 flips → 0.0% flip rate**. Gate PASSES (≤5%) — pooled-sub filler is adequate; no per-sub priors needed. Caveat: small sample, keep as a standing check as the season grows. (Mechanism also proven by the hardcoded counterexample test.)
- NOTE 2026-06-01: "The House" P&L ledger was **moved off the public scouting page to admin-only** (`GET /admin/the-house` + Admin dashboard tile; `apps/api/src/lib/house-ledger.ts`). It's an operator/validity tool and was the heaviest part of the public read. Public `/scouting` now returns only `odds`/`retrospective`/`weeks`.
