...
- **Followup T5-7g: TOCTOU contention regression test (post-impl-codex Medium).** The integration test "(o) organizer-also-scorer override" verifies path-selection identity but does NOT actually simulate concurrent UPDATEs that would trigger the narrowed-WHERE 0-rows failure mode. libsql in-memory is single-connection-effective, so a true cross-transaction contention test would require either (a) refactoring the path-selection into a unit-testable pure function, or (b) mocking tx.update to simulate "scorer_player_id changed mid-tx". Either is a follow-up; the impl-codex-rerun acknowledged the test gap as a Medium (does not block ship).
...
- **Followup T5-7d: T5-8 state-machine integration.** Replace the inline `round_states.state` read with `transitionState`-style service call when T5-8 ships. Mechanical refactor, no behavior change.
