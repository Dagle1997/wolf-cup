...
FR68: The system assigns tee rotation (blue → black → white cycle) per the calendar, holding rotation for unchecked/skipped weeks **and for rainout/cancelled rounds** — a rainout replays its assignment on the next playable Friday and the rest of the rotation shifts down

> **CORRECTED 2026-05-29 — FR68 was MIS-SPECIFIED, not changed.** Original FR68 read: *"…and **advancing** rotation for rainout/cancelled rounds."* Forensic audit of the **2024 and 2025 Final scorecards** (`reference/.../Wolf Cup 2025 Final Sheet Season Ended.xlsm`, `reference/Wolf Cup 2024 Final.xlsm`) shows the league **always HELD** the rotation across a missed week — the rained-out week's tee + side game carried to the next played week, leaving the Blue·Black·White tee cycle and the side-game cycle **unbroken**. The 2025 May-30 rainout (ghost col DA) resumed June 13 on the *due* tee+game (Black / Closest to Pin), not the next-after. So "rainouts rotate" was **never the actual rule** — the 2026 app implemented a behavior the league never used. The correct rule: **rainouts HOLD, exactly like skipped weeks** (replay the missed assignment, shift the rest). The 2026-05-29 manual prod fix RESTORED this; it did not invent a new rule. Code does NOT yet match: `apps/api/src/utils/tee-rotation.ts` advances through cancelled weeks (keys only on `is_active`); `apps/api/src/routes/admin/season.ts:297` would revert hand-shifted tees on any calendar toggle. Tracked in `_bmad-output/implementation-artifacts/tech-spec-round-tee-sidegame-override.md`. Until shipped: do not toggle the live 2026 calendar. Evidence detail: [[project_rainout_policy_flip_2026_05_29]].
