...
import {
  snapshotRoundOddsLine,
  getStoredOddsLine,
  computeRoundOddsLine,
} from "../lib/odds-line.js";
...
beforeAll(async () => {
  ...
  await db.insert(players).values([
    { id: M1, name: "Mara", createdAt: now },
    { id: M2, name: "Beck", createdAt: now },
    { id: M3, name: "Cole", createdAt: now },
    { id: M4, name: "Dale", createdAt: now },
  ...
  for (const r of SCHEDULE) {
    ...
    await db.insert(roundResults).values(
      r.lines.map((l) => ({
        roundId: r.id,
        playerId: l.playerId,
        stablefordTotal: l.stableford,
        moneyTotal: l.money,
        updatedAt: now,
      })),
    );
    // Compute harvey points over the FULL lineup (members + subs), like production.
    const pts = calculateHarveyPoints(
      r.lines.map((l) => ({ stableford: l.stableford, money: l.money })),
      "regular",
      0,
    );
    await db.insert(harveyResults).values(
      r.lines.map((l, i) => ({
        ...
        stablefordRank: 1,
        moneyRank: 1,
        stablefordPoints: pts[i]!.stablefordPoints,
        moneyPoints: pts[i]!.moneyPoints,
        updatedAt: now,
      })),
    );
  }
...
type Resp = {
  odds: Odds;
  retrospective: Retro;
  weeks: Array<{ roundId: number }>;
};

const get = async (rid: number) =>
  (await (await app.request(`/api/scouting/${rid}`)).json()) as Resp;
...
describe('odds — "The Line" on the upcoming round', () => {
  it("emits one posted line per MEMBER (no sub), sorted favorites → longshots", async () => {
    ...
    for (let i = 1; i < lines.length; i++)
      expect(lines[i - 1]!.fairProb).toBeGreaterThanOrEqual(lines[i]!.fairProb);
...
describe("house ledger is ADMIN-ONLY, off the public response", () => {
  it("the public scouting response does NOT include houseLedger", async () => {
    const raw = (await (await app.request("/api/scouting/7")).json()) as Record<
      string,
      unknown
    >;
...
describe("odds — line persistence (snapshot/freeze)", () => {
  it("snapshotRoundOddsLine stores the computed line and is idempotent", async () => {
    await snapshotRoundOddsLine(6, now);
    const stored = await getStoredOddsLine(6);
    const fresh = (await computeRoundOddsLine(6))!.odds;
    // Stored payload equals a live recompute (deterministic, same inputs).
    expect(JSON.stringify(stored)).toBe(JSON.stringify(fresh));

    // Second snapshot updates in place — one row per round (unique index).
    await snapshotRoundOddsLine(6, now + 1000);
    const rows = await db
      .select({ id: oddsLines.id })
      .from(oddsLines)
      .where(eq(oddsLines.roundId, 6));
    expect(rows).toHaveLength(1);
  });

  it("scouting serves the FROZEN stored line for a finalized round (not a recompute)", async () => {
    await snapshotRoundOddsLine(5, now);
    // Overwrite the stored payload with a sentinel a live recompute could never produce.
    await db
      .update(oddsLines)
      .set({
        payloadJson: JSON.stringify({ gated: true, reason: "SENTINEL-STORED" }),
      })
      .where(eq(oddsLines.roundId, 5));
    const { odds } = await get(5);
    expect(odds.gated).toBe(true);
    expect(odds.reason).toBe("SENTINEL-STORED"); // proves the route read the snapshot
  });

  it("a scheduled (not-yet-finalized) round has no snapshot and computes live", async () => {
    expect(await getStoredOddsLine(7)).toBeNull();
    const { odds } = await get(7);
    expect(odds.gated).toBe(false); // live compute from R1..R6
  });
});
