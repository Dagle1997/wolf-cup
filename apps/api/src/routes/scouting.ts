...
import {
  ...
  volatility,
  bestWorstHoles,
  loneWolfWhenBehindRate,
} from "@wolf-cup/engine";
import type { HoleNumber, OddsResult } from "@wolf-cup/engine";
...
import { computeRoundOddsLine, getStoredOddsLine } from "../lib/odds-line.js";
...
app.get("/scouting/:roundId", async (c) => {
  ...
  // "The Line" is built by the shared lib (one source of truth with the finalize
  // snapshot + backtest). Once the round is FINALIZED we serve the FROZEN snapshot
  // so the graded retrospective stays stable across model changes; before then we
  // compute live from form going in. Rounds finalized before snapshots existed
  // fall back to a live compute. Error-isolated: never a 500.
  const UNAVAILABLE = { gated: true as const, reason: "odds unavailable" };
  let odds: OddsResult | { gated: true; reason: string };
  if (round.status === "finalized") {
    odds =
      (await getStoredOddsLine(roundId)) ??
      (await computeRoundOddsLine(roundId))?.odds ??
      UNAVAILABLE;
  } else {
    odds = (await computeRoundOddsLine(roundId))?.odds ?? UNAVAILABLE;
  }
  ...
  if (round.status === "finalized" && targetRoster.length > 0) {
    try {
      retrospective = await buildRetrospective(
        roundId,
        targetRoster,
        nameOf,
        odds,
  ...
  const oddsOut =
    "gated" in odds && odds.gated
      ? odds
      : {
          ...odds,
          lines: (odds as Extract<OddsResult, { gated: false }>).lines.map(
            (l) => ({ ...l, name: nameOf.get(l.playerId) ?? `#${l.playerId}` }),
          ),
        };
  ...
  if (scoutedIds.length === 0 || seasonRoundIds.length === 0) {
    return c.json(
      {
        roundId,
        seasonRounds: seasonRoundIds.length,
        groups: [],
        weeks,
        odds: oddsOut,
        retrospective,
  ...
  const [resultRows, hiRows, scoreRows, decisionRows] = await Promise.all([
    ...
    db
      .select({
        ...
        playerId: roundPlayers.playerId,
        hi: roundPlayers.handicapIndex,
      })
      .from(roundPlayers)
      .where(
        and(
          inArray(roundPlayers.roundId, seasonRoundIds),
          inArray(roundPlayers.playerId, scoutedIds),
        ),
      ),
    db
      .select({
      ...
      .where(
        and(
          inArray(holeScores.roundId, seasonRoundIds),
          inArray(holeScores.playerId, scoutedIds),
        ),
      ),
    db
      .select({
      ...
      .from(wolfDecisions)
      .where(inArray(wolfDecisions.roundId, seasonRoundIds)),
  ]);
  ...
  const byPlayer = (id: number) => ({
    results: resultRows
      ...
      .sort(
        (a, b) =>
          (roundOrder.get(a.roundId) ?? 0) - (roundOrder.get(b.roundId) ?? 0),
      ),
    his: hiRows
      .filter((r) => r.playerId === id)
      .sort(
  ...
  function playerStats(id: number) {
    ...
    const perHole = new Map<
      number,
      { par: number; sumToPar: number; rounds: number; birdies: number }
    >();
    for (const s of scores) {
      ...
      const cur = perHole.get(s.holeNumber) ?? {
        par,
        sumToPar: 0,
        rounds: 0,
        birdies: 0,
    ...
    const holes = [...perHole.entries()]
      .map(([hole, v]) => ({
        hole,
        par: v.par,
        avg: Math.round((v.par + v.sumToPar / v.rounds) * 10) / 10,
      }))
      .sort((a, b) => a.hole - b.hole);
    const holesForBW = [...perHole.entries()].map(([hole, v]) => ({
      hole,
      avgToPar: v.sumToPar / v.rounds,
      rounds: v.rounds,
    }));
    const { best, worst } = bestWorstHoles(holesForBW, MIN_HOLE);
    let topBirdieHole: { hole: number; count: number; rounds: number } | null =
      null;
    for (const [hole, v] of perHole) {
      ...
      if (
        v.birdies > 0 &&
        (!topBirdieHole ||
          v.birdies > topBirdieHole.count ||
          (v.birdies === topBirdieHole.count && hole < topBirdieHole.hole))
      ) {
    ...
    for (const r of results) {
      ...
      cur.sum += r.stableford;
      cur.n += 1;
      teeAgg.set(tee, cur);
    }
    let bestTee: { tee: string; avgStableford: number; rounds: number } | null =
      null;
    for (const [tee, v] of teeAgg) {
      const avg = Math.round((v.sum / v.n) * 10) / 10;
      if (!bestTee || avg > bestTee.avgStableford)
        bestTee = { tee, avgStableford: avg, rounds: v.n };
    }
    ...
    for (const d of decisionRows) {
      ...
      if (d.decision === "alone" || d.decision === "blind_wolf")
        aloneByRound.set(d.roundId, true);
    }
    const lwb = loneWolfWhenBehindRate(
      results.map((r) => ({
        wentAlone: aloneByRound.get(r.roundId) ?? false,
        behindInMoney: (moneyByRound.get(r.roundId) ?? 0) < 0,
      })),
    );
  ...
  function rivalry(ids: number[]) {
    ...
    for (let i = 0; i < ids.length; i++) {
      for (let j = i + 1; j < ids.length; j++) {
        const a = ids[i]!,
          b = ids[j]!;
        const aMoney = new Map(
          resultRows
            .filter((r) => r.playerId === a)
            .map((r) => [r.roundId, r.money]),
        );
        const bMoney = new Map(
          resultRows
            .filter((r) => r.playerId === b)
            .map((r) => [r.roundId, r.money]),
        );
        let aWins = 0,
          bWins = 0,
        ...
        for (const [rid, am] of aMoney) {
          ...
          diff += am - bm;
          if (am > bm) aWins += 1;
          else if (bm > am) bWins += 1;
        }
        if (shared < 2) continue;
        const spread = Math.abs(aWins - bWins);
        if (
          !top ||
          spread > Math.abs(top.aWins - top.bWins) ||
          (spread === Math.abs(top.aWins - top.bWins) &&
            Math.abs(diff) > Math.abs(top.diff))
        ) {
    ...
    const aIsLeader =
      top.aWins > top.bWins || (top.aWins === top.bWins && top.diff >= 0);
    return {
  ...
  function luckyCharm(ids: number[]) {
    ...
    for (const d of decisionRows) {
      ...
      if (d.outcome === "win") cur.wins += 1;
      else if (d.outcome === "loss") cur.losses += 1;
      else if (d.outcome === "push") cur.pushes += 1;
      agg.set(k, cur);
    }
    let top: {
      aId: number;
...
async function buildRetrospective(
  ...
  const memberIds = new Set(
    roster.filter((r) => !r.isSub).map((r) => r.playerId),
  );
  ...
  const overallWinnerIsSub = hr.some(
    (h) => h.sp + h.mp === overallMax && subIds.has(h.playerId),
  );
  ...
  const anyWinnerPriced = winnerSet.some(
    (id) => lines.find((l) => l.playerId === id)?.postedAmerican != null,
  );
  ...
  return {
    ...
    winningMemberName:
      winningMemberId !== null
        ? (nameOf.get(winningMemberId) ?? `#${winningMemberId}`)
        : null,
    subSpoiled: overallWinnerIsSub,
    verdict,
    favoriteId,
    favoriteName:
      favoriteId !== null ? (nameOf.get(favoriteId) ?? `#${favoriteId}`) : null,
