...
// Hand-approved at the Story 2.4 spec gate (NFR-C1, count-only model). Edge IR =
// post-2.1a whole-dollar 1-to-1; per-player nets + totals are the approved values.
const FIXTURES = ['sandie-count.json', 'sandie-all-push.json'];

describe('sandie golden fixtures (hand-approved 2026-06-22)', () => {
  ...
  it('order-independence: sandie-count with holes REVERSED is byte-identical (stateless, NFR-C6)', () => {
    const fx = loadFixture('sandie-count.json');
