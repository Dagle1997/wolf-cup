 ...
 *     `ghinClient` export is null (matches Wolf Cup posture; the
 *     truthy-check evaluates undefined and '' identically).
 *   - KNOWN LIMITATION: state='WV' hardcoded upstream regardless of
 *     any future ?state= query param value (preserved from source for
 *     v1 byte-for-byte parity; future story can promote it).
 */
...
export const ghinClient =
  env.GHIN_USERNAME && env.GHIN_PASSWORD
    ? new GhinDirectClient(env.GHIN_USERNAME, env.GHIN_PASSWORD)
    : null;
