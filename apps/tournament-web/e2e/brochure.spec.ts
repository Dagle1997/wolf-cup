...
// Hide the dev-only vite-plugin-pwa registration badge (the app icon shown
// bottom-right in dev) — it never appears in the installed/prod app. Walk the
// whole tree incl. shadow roots; hide any small fixed/absolute element anchored
// to the bottom-right corner.
async function stripDevBadge(page: import('@playwright/test').Page): Promise<void> {
  await page.evaluate(() => {
...
test('brochure shots: leaderboard + expanded scorecard + score-entry', async ({ browser }) => {
  ...
  // (G) toggle — with an active greenie (Shooter) and polie (David) from the seed.
