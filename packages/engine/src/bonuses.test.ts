...
describe('applyBonusModifiers — 2v2 (wolf=0, partner=1)', () => {
  ...
  it('both net birdies but neither natural → 1 skin (second birdie ignored without double bonus)', () => {
    // pos0: net 3 (birdie), gross 5. pos1: net 3 (birdie), gross 6.
    // No natural birdie → double birdie doesn't fire → second birdie doesn't count.
  ...
  it('both net eagles but neither natural → 2 skins (team best level only, second eagle ignored)', () => {
    // pos0: net 2, gross 5. pos1: net 2, gross 5. No natural birdie or eagle.
    // No double bonus fires → only team's best level counts = skinCount(eagle) = 2.
  ...
  it('double birdie cancelled when opponent has net birdie (eagle+birdie vs birdie) → 2 skins (team best only)', () => {
    // Team A: pos0 net eagle (2), pos1 net birdie (3). Team B: pos2 net birdie (3), pos3 bogey (5).
    // Winner: Team A (eagle > birdie). Base: skinCount(eagle) = 2.
    // Double birdie cancelled (opponent has birdie). No per-player counting.
    const r = applyBonusModifiers(zeroBase(), [2, 3, 3, 5], [2, 3, 3, 5], NO_BONUS, WOLF(0), PARTNER(1), par);
    expect(bs(r)).toEqual([2, 2, -2, -2]);
  });

  it('double birdie cancelled when all opponents birdie → 2 skins (team best only)', () => {
    // Team A: pos0 eagle (2), pos1 birdie (3). Team B: pos2 birdie (3), pos3 birdie (3).
    // Winner: Team A (eagle > birdie). Base: skinCount(eagle) = 2.
    // Double birdie cancelled (opponent has birdie).
    const r = applyBonusModifiers(zeroBase(), [2, 3, 3, 3], [2, 3, 3, 3], NO_BONUS, WOLF(0), PARTNER(1), par);
    expect(bs(r)).toEqual([2, 2, -2, -2]);
  });

  it('double eagle cancelled when opponent has net eagle → 3 skins (team best only)', () => {
    ...
    // Base: skinCount(double_eagle) = 3.
    // Double birdie: cancelled (opponent eagle is ≤ par-1).
    // Double eagle: cancelled (opponent eagle is ≤ par-2).
  ...
  it('double bonuses apply when no opponent has birdie+', () => {
    ...
    // Base: skinCount(birdie) = 1. Double birdie fires → per-player 1+1 + bonus = 3.
  ...
  it('both teams birdie + wolf team polies → only polies and team total count ($3 each)', () => {
    // wolf=0, partner=1 both net birdie (3) with polies.
